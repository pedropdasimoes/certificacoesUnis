<template>
    <app-layout :actionButtons="true">
        <template #header> Listar Usuários {{props}}</template>
        <template #default>
            <t-alert v-if="deleted != null" class="col-span-6 md:col-span-6" color="solid-red" :closeable="true"
                :radius="5" :timer="5000">
                <b>{{ deleted }}</b>
            </t-alert>
            <t-table :content="content" :header="header" :searchable="['name']" color="solid-blue"
                ...
                <template #id="{ props }">
                    <t-dropdown color="light-blue" size="fit">
                        ...
                        <t-dropdown-item v-if="user != props.id" :link="route('editar-usuarios', [props.id])">
                            <!--You can use any icon component but you must use slot="icon" attributes-->
                            <span class="mr-1.5">
                        ...
                        <t-dropdown-item v-if="user != props.id" @click.native="
                            () => {
